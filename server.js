...
// --- LISTA COMPLETA E GIGANTE RESTAURADA ---
const LISTA_ATIVOS = [
...
app.get('/', (req, res) => {
    ...
    let cardsHtml = slots.map((id, i) => {
        ...
        return `<div style="background:#111418; padding:15px; border-radius:15px; border:1px solid #333; opacity:${id === "NONE" ? '0.5' : '1'}">
            ...
            <select onchange="window.location.href='/mudar/${i}/'+this.value" style="background:#000; color:#fff; border:1px solid #444; width:100%; font-size:11px">
                <option value="">Trocar/Desativar...</option>${optionsHtml}
            </select>
        </div>`;
    }).join('');

    res.send(`<!DOCTYPE html><html><head><title>KCM V19</title><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="refresh" content="5">
    ...
    <body><h3 style="font-style:italic">K.C<span style="color:#1e90ff">📈</span>M ULTIMATE V19</h3>
    <div style="width:100%; max-width:450px; background:#0a0c0f; border:2px solid #0047AB; border-radius:20px; padding:15px">
        ...
        <div style="background:#000; padding:10px; border-radius:10px; border:1px solid #222; font-size:12px">
            <div style="color:#1e90ff; font-weight:bold; margin-bottom:8px">📊 PLACAR GLOBAL: ${globalStats.winDireto + globalStats.winGales}W - ${globalStats.loss}L</div>
